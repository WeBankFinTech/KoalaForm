import { FMessage, FSpin } from '@fesjs/fes-design';
import {
    ComponentDesc,
    ComponentType,
    composeRender,
...
export const mapTableFields = (fields: Field[], comm?: Field) => {
    return fields.map((item) => {
        ...
        if ((components as ComponentDesc)?.['name'] === ComponentType.DatePicker && !field.format) {
...
export const useCurd = (config: CurdConfig) => {
    ...
    const openModal = (type: 'create' | 'update' | 'view', record?: any) => {
        ...
        if (data) {
            editCfg?.fields.forEach((field) => {
                const comp = (field.components as Array<ComponentDesc>)?.[0] || field.components;
                if (!comp) return;
    ...
    useTable({
        ...
        table: merge<ComponentDesc, ComponentDesc>(
            ...
            tableCfg.table as ComponentDesc,
        ),
        fields: [
    ...
    const render = (slots: Slots) => {
        const cr = composeRender([query.render, spin.render, pager.render, modalCfg && modal.render].filter(Boolean) as SceneContext['render'][]);
        return cr(slots);
    };
